...
class Timeseries:
    ...
    def __query(self, q, params={}, _params={}):
        q_prefix = """
            from(bucket:"{bucket}")
        """.format(bucket=Config.INFLUX_BUCKET)
    ...
    def get_points_agg(self, start=timedelta(days=-1), end=None, aggregate="mean", every=timedelta(hours=1)):
        if not end:
            end = datetime.now()
