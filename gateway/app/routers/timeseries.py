...
from qshed.client.models.response import TimeseriesResponse, TimeseriesListResponse, error
...
async def create(timeseries_request: Request):
    ...
    sql_timeseries = sqlTimeseries.create(
        name=timeseries.name
    )
...
async def add(timeseries_request: Request):
    ...
    if timeseries.id is not None:
        ...
        sql_timeseries = (
            sql_session
            .query(sqlTimeseries)
            .filter_by(name=timeseries.name)
            .first()
        )
        if sql_timeseries is None:
            raise HTTPException(
